...
test(`README: Find all HackerNews posts which have more than 50 comments`, () => {
    const { expression } = NodeSet.any()
        ...
        .filter(NodeSet.element(`tr`, `following-sibling`)
    ...
    expect(expression).toBe(`descendant-or-self::node()[attribute::class[contains(concat(" ", normalize-space(self::node()), " "), " athing ")]][following-sibling::tr[(position() = 1)] / child::td[2] / child::a[last()][(substring-before(child::text(), "\u00A0") > 50)]]`);
});
test(`README: Select a set of nodes`, () => {
    expect([
        ...
        NodeSet.element(`chapter`, `following-sibling`).filter(fn(`position`).is(`=`, 1)),
        // selects the previous chapter sibling of the context node
        NodeSet.element(`chapter`, `preceding-sibling`).filter(fn(`position`).is(`=`, 1)),
        ...
        NodeSet.element(`chapter`).filter(NodeSet.element(`title`).is(`=`, `Introduction`)),
        ...
        NodeSet.element(`*`).filter(NodeSet.element(`chapter`, `self`).or(NodeSet.element(`appendix`, `self`))),
        // selects the last chapter or appendix child of the context node
        NodeSet.element(`*`)
            .filter(NodeSet.element(`chapter`, `self`).or(NodeSet.element(`appendix`, `self`)))
            .filter(fn(`position`).is(`=`, fn(`last`))),
    ].map(({ expression }) => expression)).toEqual([
        `child::para`,
        `child::*`,
        `child::text()`,
        `child::node()`,
